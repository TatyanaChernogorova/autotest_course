...
def scrabble(word):
    """Подсчт количества очков, полученное за слово"""
    new_dict = {
        1: "авеёинорст",
        2: "дклмпу",
        3: "бгья",
    ...
    points = 0
    for letter in word:  # перебор списка, который поступает на вход
        for key, value in new_dict.items():  # перебор ключей и значений в словаре
            if letter in value:  # сравнение буквы из списка с буквой из значения словаря
                points += key
    return points
