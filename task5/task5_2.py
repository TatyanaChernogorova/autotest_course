...
def repeats(our_str):
    ...
    dictionary = dict()
    new_str = ""
    for elem in our_str:  # перебор списка our_str, который поступает на вход
        dictionary[elem] = dictionary.get(elem, 0) + 1  # подсчет количества вхождений буквы
        new_str += f'{elem}_{dictionary[elem]}'
    return new_str
