...
def to_roman(val):
    ...
    roman_str = ""
    for date in date_dict:  # перебор словаря date_dict
        while val >= date:  # сравнение даты которые поступают на вход с ключем словаря
            roman_str += date_dict[date]  # конкатенация значений словаря
            val -= date
    return roman_str
