...
def create_phone_number(num_tuple):
    ...
    slice_last_elements = ''.join(map(str, num_tuple[6:]))
    slice_first_elements = ''.join(map(str, num_tuple[:3]))
    slice_middle_elements = ''.join(map(str, num_tuple[3:6]))
    str_phone = f'({slice_first_elements}) {slice_middle_elements}-{slice_last_elements}'
    return str_phone
